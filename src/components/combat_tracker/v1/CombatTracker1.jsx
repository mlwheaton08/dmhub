...
import styles from "./CombatTracker1.module.css"
import { conditions_dict } from '../../../utils/dictionaries'
import { emptySlot, defaultColumns } from '../../../utils/constants'
import { useNavigate } from "react-router-dom"

export function CombatTracker1() {

    // initiative bonus
    // button on each slot to clear all conditions
        // also little x on each condition on hover to clear it
    // concentration checkbox
        // prolly doesn't even need to do anything. just a functionless checkbox
    // easy way to add/subtract hp, so user doesn't have to do math
    // noel says let user color each row how they want
        // also prolly have a few color themes to choose :D
    // maybe number rows?
    // also count rounds of combat
        // prolly a later thing, if it even happens. idk how useful this would actually be
            // main use would be to track conditions and how long someone has been concentrating
    ...
    // maybe have a section for general battle notes. like envirmonment/triggers
    // all (or most) icons should have a message on hover saying what it is
    // think about pulling from a dnd api (i think i found a solid free one)
        // i know it's a bigger goal, but it's realllly nice to have monster stat blocks right there (like dnd beyond). i could make my own
            // also have my stat blocks editable! i like altering stats and this would make it real easy
    // option to duplicate a row? for quicker entry. maybe the duplicated name adds the classic (1) to it. or if it already has (num), then add to that number
    // general note: after quickly browsing comments on a reddit post, i think many people avoid online intiative trackers because they prove to be more of a hassle than just pen/paper. so i think a focus for this app should be that it's incredibly simple/fast to use. people think spreadsheets are a simple way to track this? hold my beer
        // to add to this, i think people really like spreadsheets for how quickly you can edit them. My table layout maybe lacks a bit in visual identification (easily spotting sections), but it's easy to maneuver (tab and enter for navigating) which is a HUGE plus. don't forget that.
            // on second thought, dndbeyond's tracker isn't a table and it's really nice. if i have usefull buttons and input navigation, i might not need a table. idk. i just really wanna fit more info than i feel like a row in a table can
                // to add to this, i think a notepad is obviously quicker/easier than even the simplest table to track combat. and the app separates what you're looking at. so having more info/functionality (statblock, rolling, etc) makes it more handy
                // also, dndbeyond's tracker seems awesome but it only allows like 8 monsters or something. so mine can be the clutch free version of that :)
            // i'm honestly torn. visually the non-table layout is way better. the table layout is such a nice quick "jot it down" basic layout. i'm leaning towards the non-table layout though because there's a lot of useful info to have on the fly. Maybe i can clean this code up a bit and save it as v1? Then make the non-table layout v2
